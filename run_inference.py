...
def drop_bad_rows(df, filename):
    ...
    col_map = {
        "scd_co2": ["scd_co2", "scd_c"],
        "scd_temp": ["scd_temp", "scd_t"],
        "scd_hum": ["scd_hum", "scd_h"],
        "bme_temp": ["bme_temp", "bme_t"],
        "bme_hum": ["bme_hum", "bme_h"],
        "bme_pres": ["bme_pres", "bme_p"],
    ...
    mask_valid = (df["scd_co2"] > 100) & (df["scd_hum"] >= 0) & (df["scd_hum"] <= 100)
    df = df[mask_valid]
    ...
    if dropped > 0:
        print(
            f"    🗑️ {dropped} fehlerhafte Zeilen gelöscht ({(dropped / initial_count) * 100:.1f}%)."
        )
    else:
...
def process_file(filename, scaler_gas, scaler_env, foundation, classifier):
    ...
    if len(df) < SEQ_LEN:
        print(
            f"    ❌ Zu wenig Daten übrig nach Bereinigung ({len(df)} < {SEQ_LEN}). Überspringe Datei."
        )
        return None
    ...
    with torch.no_grad():
        for i in range(len(df)):
            ...
            if mold_prob > 0.8:
                preds.append("SCHIMMEL")
            elif mold_prob > 0.5:
                preds.append("VERDACHT")
            else:
                preds.append("OK")
