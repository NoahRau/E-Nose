...
def main():
    if not DATA_FILE.exists():
        ...
        print(
            "Bitte führe erst das 'run_inference.py' Skript aus, um die Vorhersagen zu generieren."
        )
        return
    ...
    _fig, axes = plt.subplots(n_files, 1, figsize=(15, 5 * n_files), sharey=True)
    if n_files == 1:
        axes = [axes]  # Falls nur 1 Datei, in Liste packen
    ...
    for i, file_name in enumerate(files):
        ...
        ax.axhline(
            0.8, color="red", linestyle="--", linewidth=1.5, label="Schimmel (>0.8)"
        )
        ax.axhline(
            0.5, color="orange", linestyle="--", linewidth=1.5, label="Verdacht (>0.5)"
        )
