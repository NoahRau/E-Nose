    ...
    def __init__(self, root_dir, seq_len=512, scaler_path="scaler.pkl"):
        ...
        if not Path(scaler_path).exists():
            raise FileNotFoundError(
                f"Scaler {scaler_path} fehlt! Erst Phase 1 trainieren."
            )
...
def main():
    ...
    # 1. Dataset
    dataset = LabeledFridgeDataset(
        LABELED_DATA_DIR, seq_len=SEQ_LEN, scaler_path=SCALER_PATH
    )
    if len(dataset) == 0:
    ...
    foundation = FridgeMoCA_V3(gas_chans=n_gas, env_chans=n_env, seq_len=SEQ_LEN).to(
        device
    )
    ...
    if FOUNDATION_MODEL_PATH.exists():
        ...
        foundation.load_state_dict(
            torch.load(FOUNDATION_MODEL_PATH, map_location=device)
        )
    else:
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for gas, env, labels in val_loader:
                ...
                features = foundation.forward_features(gas, env)
                val_correct += (
                    (classifier(features[:, 0]).argmax(1) == labels).sum().item()
                )
                val_total += labels.size(0)

        logger.info(
            f"Ep {epoch + 1}: Loss={total_loss / len(train_loader):.4f} | "
